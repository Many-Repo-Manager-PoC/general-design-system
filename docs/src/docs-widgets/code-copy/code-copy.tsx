import { type PropsOf, type Signal, component$, useSignal } from "@qwik.dev/core";
// import { Button } from "general-design-system";

export type CodeCopyProps = PropsOf<"button"> & {
  preRef?: Signal<HTMLPreElement | undefined>;
};

export const CodeCopy = component$<CodeCopyProps>(({ preRef, ...props }) => {
  const copied = useSignal(false);

  return (
    <button
      {...props}
      title={copied.value ? "Copied to Clipboard" : "Copy to Clipboard"}
      class={props.class}
      onClick$={async () => {
        await navigator.clipboard.writeText(preRef?.value?.textContent ?? "");
        copied.value = true;

        setTimeout(() => {
          copied.value = false;
        }, 4000);
      }}
    >
      {!copied.value ? (
        <CopyIcon />
      ) : (
        <div class="flex gap-1">
          <ClipboardCheck />
        </div>
      )}
    </button>
  );
});

export function CopyIcon(props: PropsOf<"svg"> & { color?: string }, key: string) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="1rem"
      height="1rem"
      viewBox="0 0 16 16"
      class="group"
      {...props}
      key={key}
    >
      <title>Copy to Clipboard</title>
      <path
        class="fill-slate-300 group-hover:fill-slate-100"
        d="M0 6.75C0 5.784.784 5 1.75 5h1.5a.75.75 0 0 1 0 1.5h-1.5a.25.25 0 0 0-.25.25v7.5c0 .138.112.25.25.25h7.5a.25.25 0 0 0 .25-.25v-1.5a.75.75 0 0 1 1.5 0v1.5A1.75 1.75 0 0 1 9.25 16h-7.5A1.75 1.75 0 0 1 0 14.25Z"
      />
      <path
        class="fill-slate-300 group-hover:fill-slate-100"
        d="M5 1.75C5 .784 5.784 0 6.75 0h7.5C15.216 0 16 .784 16 1.75v7.5A1.75 1.75 0 0 1 14.25 11h-7.5A1.75 1.75 0 0 1 5 9.25Zm1.75-.25a.25.25 0 0 0-.25.25v7.5c0 .138.112.25.25.25h7.5a.25.25 0 0 0 .25-.25v-7.5a.25.25 0 0 0-.25-.25Z"
      />
    </svg>
  );
}

export function ClipboardCheck(props: PropsOf<"svg">, key: string) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="1rem"
      height="1rem"
      viewBox="0 0 16 16"
      {...props}
      key={key}
    >
      <title>Copied to Clipboard</title>
      <path
        fill="white"
        d="M13.78 4.22a.75.75 0 0 1 0 1.06l-7.25 7.25a.75.75 0 0 1-1.06 0L2.22 9.28a.751.751 0 0 1 .018-1.042a.751.751 0 0 1 1.042-.018L6 10.94l6.72-6.72a.75.75 0 0 1 1.06 0Z"
      />
    </svg>
  );
}
